...
class ICBM152BEM(Config):
    ...
    def __init__(self, **kwargs):
        self.cache_path_icbm: str = op.join(appdirs.user_cache_dir("cerebra_atlas_python"), "ICBM152")
        ...
        self.bem_conductivity_string = "bem_" + "".join([str(x) + "_" for x in self.bem_conductivity])[:-1]
        self.bem_name = f"{self.bem_conductivity_string}_ico_{self.bem_ico}"
        self._bem_path = op.join(
            self.cache_path_icbm, f"{self.bem_name}_bem.fif"
        )
        # Output paths
        self._bem_model_path = op.join(self.cache_path_icbm,f"{self.bem_name}.pkl",)
